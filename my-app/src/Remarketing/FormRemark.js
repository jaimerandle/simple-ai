...
import { Box, Button, TextField, InputLabel, Select, MenuItem, FormControl, Grid, Tooltip, IconButton, Typography, Modal, CircularProgress } from '@mui/material';
import { createPeriodicJob, getPeriodicJobs, updatePeriodicJob } from '../services/bffService';
...
const Formulario = ({ onCampaignSaved, updateCampaigns }) => {
    ...
    const scheduleToFrequency = (schedule) => {
        ...
        if (parts.length === 6) { // Cron con segundos
        ...
        } else if (parts.length === 5) { // Cron sin segundos
            if (parts[1].startsWith('*/')) {
                const hours = parseInt(parts[1].substring(2), 10);
                if (hours === 24) return { frequency: 'allDays', dayOfWeek: '' };
    ...
    const handleSendCampaign = async () => {
        if (checkChanges() && !isFirstRender.current) {
            setOpenModal(true);
        } else {
        ...
        if (message.trim()) {
            try {
                ...
                const params = {
                    ...
                    params: {
                        clientId: clientId,
                        assistantId: assistant[0].oai_assistant_id,
                        channelType: 7,
                ...
                if (campaignId) {
                    ...
                    await createPeriodicJob(clientId, title, params, token, schedule);
                }
                ...
                <>
                    ...
                    <Box sx={{ display: 'flex', width: '100%', alignItems: 'center', marginBottom: '16px' }}>
                        <TextField
                            ...
                            fullWidth
                            size="small"
                            sx={{ flexGrow: 1, marginRight: '8px', '& .MuiFormHelperText-root': {
                                color: 'black',
                            }, }}
                        />
                    ...
                    <Button
                        ...
                        sx={{ width: '40%', marginBottom: '16px', backgroundColor: '#1976d2', color: 'white', '&:hover': { backgroundColor: '#1565c0' } }}
                    >
                        Enviar Campaña
                    </Button>
                </>
