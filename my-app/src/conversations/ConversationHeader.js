    ...
    <Box sx={{ padding: 2, marginTop: "4px",width:isMobile?"100%":"30%", textAlign:"left"}}>
        ...
        <>
          <Box sx={{display:"flex",justifyContent:"space-between"}}>
            <Box>
              <Typography  style={{marginBottom:"8%"}} color="#6728B8" ><strong>ID: {conversation.id} </strong></Typography>
              <div style={{display:"flex",marginBottom:"20%"}}>
                <div>
                  <Typography  color="#867F8A"><strong>Canal:</strong></Typography>
                  <Typography  color="#867F8A"><strong>Fecha:</strong></Typography>
                  <Typography color="#867F8A"><strong>Hora:</strong></Typography>
                </div>
                <div>
                  <Typography  color="#867F8A">{canal === "MELI" ? "Mercado Libre" : canal}</Typography>
                  <Typography   color="#867F8A">{new Date(conversation.last_updated).toLocaleDateString()}</Typography>
                  <Typography color="#867F8A">{new Date(conversation.last_updated).toLocaleTimeString()}</Typography>
                </div>
              </div> 
            </Box>
            <Box sx={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0px"}}>
              <div style={{ display: "flex", justifyContent: "center", width: isMobile ? "70%" : "50%", marginTop: "10px", marginBottom: "-10px" , border:"1px solid grey",borderRadius:"10px"}} >
            ...
            <Button style={{color:"transparent", color:"#EB4335", width:"30%", fontSize:"5px", height:"50px", display:"inline-grid" }} onClick={handleOpenDeleteDialog} sx={{ mt: 2 }}>
              <img style={{height:"30px",}} src={Delete} alt=""/>
              {isMobile?"":"Eliminar conversación"}
              
            </Button>
            <Button onClick={handleOpenNoteDialog} sx={{ mt: 2, ml: 2 }}  style={{color:"transparent", color:"grey", width:"30%", fontSize:"5px", height:"50px", display:"inline-grid" }}>
            <img style={{height:"30px"}} src={Notes} alt=""/>
              {isMobile?"":"Ver Notas"}
            </Button>
          </div>
